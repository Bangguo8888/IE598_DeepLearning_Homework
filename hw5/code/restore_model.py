...
def bn(x, c):
    return tf.contrib.layers.batch_norm(x, is_training=c['is_training'], center=True, scale=True, decay=BN_DECAY,
...
saver.restore(sess, tf.train.latest_checkpoint('./sigle_frame_model1/'))
...
import pickle
with open('output/class_list.pkl','wb') as f:
    pickle.dump(class_list, f)
print('save class_list successfully')
...
np.save('output/confusion_matrix',confusion_matrix)
